...
export async function getReusablePlayBlueprints(archiveId, limit = 12) {
  ...
  for (const row of rows || []) {
    ...
    if (Number(state.version || 0) < 7 || state.aiStrategy !== "compact-spine-local-final") continue;
    if (!Array.isArray(state.scenePlan) || state.scenePlan.length !== 4 || !state.publicOpening?.firstScene) continue;
    const allScenes = [state.publicOpening.firstScene, ...state.scenePlan];
    if (!allScenes.every((scene) => scene?.focusObject && scene?.anomaly && Array.isArray(scene?.options) && scene.options.length >= 3)) continue;
