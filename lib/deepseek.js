...
function parseJsonLoose(text) {
  ...
  const candidates = [value, value.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, ""), value.includes("{") && value.includes("}") ? value.slice(value.indexOf("{"), value.lastIndexOf("}") + 1) : ""];
...
function asSystemMessages(system) {
  return (Array.isArray(system) ? system : [system]).filter((part) => String(part || "").trim()).map((content) => ({ role: "system", content: String(content) }));
}
function cacheStats(usage) {
  ...
  return { promptTokens: prompt, completionTokens: completion, cacheHitTokens: hit, cacheMissTokens: miss, cacheHitRate: hit + miss > 0 ? hit / (hit + miss) : 0 };
}
async function oneCall({ model, system, user, thinking, reasoningEffort, maxTokens, userId }) {
  ...
  if (process.env.DEEPSEEK_CACHE_DEBUG === "true") {
    console.info("DeepSeek usage", { model: payload?.model || model, promptTokens: cache.promptTokens, completionTokens: cache.completionTokens, cacheHitTokens: cache.cacheHitTokens, cacheMissTokens: cache.cacheMissTokens, cacheHitRate: Number(cache.cacheHitRate.toFixed(3)) });
  }
...
export async function callDeepSeek(options) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try { return await oneCall(options); }
    catch (error) {
      lastError = error;
...
export function fastModel() { return process.env.DEEPSEEK_FAST_MODEL || "deepseek-v4-flash"; }
export function proModel() { return process.env.DEEPSEEK_PRO_MODEL || "deepseek-v4-pro"; }
